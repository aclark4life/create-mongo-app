...
def _hash_token(token: str) -> str:
    ...
    Refresh tokens are high-entropy JWTs, so a fast SHA-256 digest is sufficient
    and constant work. Storing the hash (never the plaintext) means a read-only
    leak of the token collection cannot be replayed against /login/refresh.
    """
...
class CRUDToken(CRUDBase[Token, RefreshTokenCreate, RefreshTokenUpdate]):
    ...
    async def create(self, db: AsyncDatabase, *, obj_in: str, user_obj: User) -> Token:
        token_hash = _hash_token(obj_in)
        db_obj = await self._collection(db).find_one({"token": token_hash})
        if db_obj:
            ...
            new_token = self.model(token=token_hash, authenticates_id=user_obj.id)
    ...
    async def get(self, db: AsyncDatabase, *, user: User, token: str) -> Token | None:
        doc = await self._collection(db).find_one({"token": _hash_token(token), "authenticates_id": user.id})
